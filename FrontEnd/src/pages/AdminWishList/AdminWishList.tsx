...
const AdminWishList = () => {
    ...
    const columns: GridColDef[] = [
        { field: 'userId', headerName: 'ID', width: 330 },
        {
            ...
            renderCell: (params) => (
                <div className="img">
                    <img src={`http://localhost:3001/${users.find((x) => x._id == params.row.userId)?.posts.find((z) => z.id == params.row.postId)?.img}`} alt="" />
                </div>
            )
        },
        {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            ...
                            axios.patch(`http://localhost:3001/api/users/${id}`,{
                                wishList:user?.wishList.filter((x:{postId:string})=>x.postId !=params.row.postId)
                            }).then(() => {
                                dispacth(getAllData())
                            })
                        }
